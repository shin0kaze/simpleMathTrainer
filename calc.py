...
def __start_game(quiz_data, operation, difficulty, modification):
    ...
    *data_to_save, answers = start_quiz('test', qfunc, 1000, modification, operation)
    if answers:
...
def open_traincalc_window():
    ...
    layout = [  
        ...
        [sg.Combo(quiz_op_type, key = c_quiz, default_value=quiz_op_type[0]),
            sg.Combo(tasks.EToList(quiz_mod), key = c_mod, default_value=tasks.EToList(quiz_mod)[0]),
